...
def _parse_abstract(root: etree._Element) -> list[Paragraph]:
    ...
    div_paragraphs = abstract_elem.findall("tei:div/tei:p", NS)
    if div_paragraphs:
...
def _parse_article_meta_tei(root: etree._Element, doc: Document) -> None:
    ...
    journal_el = root.find(".//tei:sourceDesc//tei:monogr/tei:title[@level='j']", NS)
    ...
    if monogr is not None:
        # Volume
        vol_el = monogr.find("tei:imprint/tei:biblScope[@unit='volume']", NS)
        if vol_el is not None:
            doc.volume = text(vol_el)
        # Issue
        issue_el = monogr.find("tei:imprint/tei:biblScope[@unit='issue']", NS)
        if issue_el is not None:
            doc.issue = text(issue_el)
        # Pages
        page_el = monogr.find("tei:imprint/tei:biblScope[@unit='page']", NS)
...
def _parse_inline(elem: etree._Element, parts: list[str], refs: list[InlineRef]) -> None:
...
def _parse_annex(root: etree._Element) -> list[Section]:
    ...
    sections: list[Section] = []
    annex_div = root.find(".//tei:back/tei:div[@type='annex']", NS)
...
def _parse_additional_back(root: etree._Element) -> list[Section]:
    ...
    for div in back.findall("tei:div", NS):
        ...
        if section.paragraphs or section.subsections or section.heading:
            sections.append(section)
...
def _parse_bibliography(root: etree._Element) -> list[Reference]:
    ...
    references = []
    for idx, bib in enumerate(root.findall(".//tei:back//tei:listBibl/tei:biblStruct", NS)):
...
def _parse_bib_authors(
    bib_elem: etree._Element,
    ref: Reference,
) -> None:
    """Extract authors from a ``<biblStruct>`` into *ref*."""
    for author_elem in bib_elem.findall(".//tei:author/tei:persName", NS):
...
def _parse_bib_imprint(
    ...
    for scope in bib_elem.findall(".//tei:monogr/tei:imprint/tei:biblScope", NS):
        unit = scope.get("unit", "")
        if unit == "volume":
    ...
    date_elem = bib_elem.find(".//tei:monogr/tei:imprint/tei:date", NS)
...
def _parse_bib_editors(
    ...
    for editor in bib_elem.findall(".//tei:monogr/tei:editor/tei:persName", NS):
...
def _parse_bib_entry(bib_elem: etree._Element, index: int) -> Reference:
    ...
    title_elem = bib_elem.find(".//tei:analytic/tei:title[@level='a']", NS)
    if title_elem is not None:
        ref.title = all_text(title_elem)

    # Journal — fall back to monograph title for books/proceedings
    journal_elem = bib_elem.find(".//tei:monogr/tei:title[@level='j']", NS)
    if journal_elem is not None:
        ref.journal = all_text(journal_elem)
    if not ref.journal:
        monogr_title = bib_elem.find(".//tei:monogr/tei:title[@level='m']", NS)
    ...
    # Publisher
    pub_elem = bib_elem.find(".//tei:monogr/tei:imprint/tei:publisher", NS)
    if pub_elem is not None:
        ref.publisher = text(pub_elem)

    # Publisher location
    pub_loc = bib_elem.find(".//tei:monogr/tei:imprint/tei:pubPlace", NS)
